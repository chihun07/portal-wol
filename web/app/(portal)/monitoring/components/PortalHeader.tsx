...
function renderLink(link: PortalLink, onNavigate?: () => void) {
  if (link.external) {
    return (
      <a className={`btn ghost${link.accent ? ' portal-link--cta' : ''}`} href={link.href} target="_blank" rel="noopener">
        {link.label}
      </a>
    );
  }
  ...
  return (
    <Link className={`btn ghost${link.accent ? ' portal-link--cta' : ''}`} href={link.href as any} onClick={onNavigate}>
      {link.label}
    </Link>
  );
}
  ...
  return (
    <header className="portal-header">
      <div className="portal-headbar">
        ...
        <Link className="btn ghost settings-link" href="/settings?returnTo=/">
          {settingsLabel}
        </Link>
